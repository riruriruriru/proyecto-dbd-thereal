...
<div class="form-group">
  ...
  <input id="cantidad_voluntarios" name="cantidad_voluntarios" placeholder="Cantidad Voluntarios" class="form-control" min=1 required="" type="number"> 
</div>

<!-- Text input-->
<div class="form-group">
  <label  for="monto_objetivo">Monto Objetivo</label> 
  <input id="monto_objetivo" name="monto_objetivo"  placeholder="Monto Objetivo" class="form-control" required="" min=1 type="number">
...
<?php
        $dato = Session::get('flash');
        if ($dato != ''){
            if ($dato == 'Evento declarado correctamente'){
              echo "<script>alert('";
              echo $dato;
              echo "')";
              echo "</script>";
